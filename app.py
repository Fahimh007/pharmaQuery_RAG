...
def get_embedding_model(api_key):
    """Initialize and cache the embedding model with the provided API key."""
    if not api_key:
...
def run_rag_chain(query):
    ...
    api_key = get_gemini_api_key()
    if not api_key:
        return "API key is required. Please enter your Gemini API key in the sidebar."
    ...
    chat_model = ChatGoogleGenerativeAI(
        # Use a model that is typically available on free tier.
    ...
    try:
        ...
        # Surface quota/model issues in a cleaner way for Streamlit users.
        err_text = str(exc)
        if "RESOURCE_EXHAUSTED" in err_text or "429" in err_text:
            return (
                "Gemini API quota exceeded for the selected model. "
                "Please wait a minute and retry, switch to a lower-cost model "
                "(e.g., gemini-2.0-flash), or enable billing in Google AI Studio."
            )
        return f"Gemini API error: {err_text}"
